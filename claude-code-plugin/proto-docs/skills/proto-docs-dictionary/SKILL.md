...
1. Global field-level Dictionary candidate
   - Path: `docs/dictionary/candidates/word-dictionary.json`
...
2. Message-level Dictionary candidates
   - Path: `docs/dictionary/candidates/messages/<MessageName>.json`
...
- If an override is needed, promote the reviewed override entry from `inference.candidate_override` to `message_dictionary_override` with `npm run proto-docs -- promote-candidate-override -- --candidates docs/dictionary/candidates --message <MessageName> --field <field_name>`.
...
```bash
npm run proto-docs -- validate-candidates -- --candidates docs/dictionary/candidates
```
...
```bash
npm run proto-docs -- promote-candidate-override -- --candidates docs/dictionary/candidates --message SettlementLine --field asset_key
```
...
- Read `docs/dictionary/word-dictionary.json`.
- Run `npm run proto-docs -- generate-candidates -- --proto <path>`.
- Run `npm run proto-docs -- validate-candidates -- --candidates docs/dictionary/candidates`.
- Create or update `docs/dictionary/candidates/word-dictionary.json` and files under `docs/dictionary/candidates/messages/`.
- Promote approved candidate scopes using `dictionary-manager.js promote-candidates` with approval evidence.

## Forbidden

- Directly edit `docs/dictionary/word-dictionary.json` without approval evidence.
